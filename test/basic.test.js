...
it("should pass with a basic mock", () =>
  fetchception(
    [
      {
        request: "/api/foo",
...
it("should allow passing a single exchange pair directly", () =>
  fetchception(
    {
      request: "/api/foo",
      response: {
...
it("should fail if any requests are made with no exchanges defined (passed as undefined)", () => {
  return expect(
    () => fetchception(undefined, () => fetch("/api/foo")),
    "to error",
    [
      "expected",
      "GET /api/foo",
...
it("should accept a statusCode as response shorthand", () =>
  fetchception(
    {
      request: "GET /foo",
      response: 500
...
it("should allow specifying the expected query string via the `query` option", function() {
  return expect(
    () =>
      fetchception(
        [{ request: { query: { foo: ["bar", "quux"] } }, response: 200 }],
        () => fetch("/api/bar?foo%5B0%5D=bar&foo%5B1%5D=baz")
      ),
    "to error",
    "expected\n" +
